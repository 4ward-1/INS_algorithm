...
def load_data(id = 0):
    """Загрузка датасета (показаний акселерометров, гироскопов, магнитомеотров и GPS координат)\nid - идентификатор датасета (по умолчанию 0)"""
    accelerometer = pd.read_csv('%s_TotalAcceleration.csv' % id)
    gyroscope     = pd.read_csv('%s_Gyroscope.csv' % id)
...
def constants():
    """Формирование словаря констант, используемых в алгоритме"""
...
def razb(accelerometer, gyroscope, magnetometer, GPS):
    """Разбиение данных из датасета на составляющие:\nОсь X(E) - поперечная ось (вправо в плоскости экрана), Y(N) - продольная ось (вверх в плоскости экрана), Z(h) - вертикальная ось (перпендикулярно плоскости экрана)\nB - широта, L - долгота
Для акселерометров ускорение в сторону оси - положительное значение. Для гироскопов - при вращении против часовой стрелки (если смотреть с конца оси) - положительное значение.
Формирование массивов времени для каждого измерителя"""
...
def interpolation(time_acc, acc_x, acc_y, acc_z, time_gyro, gyro_x, gyro_y, gyro_z, time_magn, magn_x, magn_y, magn_z, time_GPS, B, L):
    """Интерполяция всех измерений на одно время"""
...
def initial_cond(DT, acc_x_i, acc_y_i, acc_z_i, B, L, magn_x_i, magn_y_i, magn_z_i):
    """Алгоритм начальной выставки - формирование начальных значений скорости, координат, дифферента, крена, курса для алгоритма ИНС"""
    ...
    A2 = Matrix([[1, 0, 0], [0, sym.cos('theta'), sym.sin('theta')], [0, sym.sin('-theta'), sym.cos('theta')]])
    A3 = Matrix([[sym.cos('gam'), 0, sym.sin('-gam')], [0, 1, 0], [sym.sin('gam'), 0, sym.cos('gam')]])
    A1 = A3 * A2
    ...
    c1_22 = C10_val[1, 1];
    c1_23 = C10_val[1, 2]
    magn_E1i = c1_11 * np.mean(magn_x_i[:int(1 / DT)]) + c1_12 * np.mean(magn_y_i[:int(1 / DT)]) + c1_13 * np.mean(
        magn_z_i[:int(1 / DT)])
    magn_N1i = c1_21 * np.mean(magn_x_i[:int(1 / DT)]) + c1_22 * np.mean(magn_y_i[:int(1 / DT)]) + c1_23 * np.mean(
        magn_z_i[:int(1 / DT)])
    ...
    return V_Ei, V_Ni, V_hi, dS_Ei, dS_Ni, dS_hi, B0, L0, theta0, gam0, K0

def matrix_form():
    """Формирование аналитических зависимостей для матриц направляющих косинусов A (перехода от ENh к XYZ), C (перехода от XYZ к ENh),
для вектора ускорения силы тяжести в ENh, XYZ и вектора напряженности магнитного поля Земли в ENh, XYZ,
для производных элементов матрицы C"""
    ...
    G = Matrix([[0], [0], ['g']])
    Mgn = Matrix([[0], ['m_N'], ['m_h']])
    G_xyz = A * G
    Mgn_xyz = A * Mgn
    CC = Matrix([['c11', 'c12', 'c13'], ['c21', 'c22', 'c23'], ['c31', 'c32', 'c33']])
    Om_xyz_kos = Matrix([[0, '-om_z', 'om_y'], ['om_z', 0, '-om_x'], ['-om_y', 'om_x', 0]])
    dC = CC * Om_xyz_kos
    return A, C, G, Mgn, G_xyz, Mgn_xyz, CC, dC

if __name__ == '__main__':
    flag_alg = input('Выбереите алгоритм обработки исходных данных:\n 1 - Гиромагнитная курсовертикаль (уравнения Пуассона) \n'
                         ' 2 - Навигация без коррекции по GPS (уравнения Пуассона) \n'
                         ' 3 - Навигация с коррекцией по GPS (уравнения Пуассона) \n')
    ...
    [A, C, G, Mgn, G_xyz, Mgn_xyz, CC, dC] = matrix_form()
    [V_Ei, V_Ni, V_hi, dS_Ei, dS_Ni, dS_hi, B0, L0, theta0, gam0, K0] = initial_cond(DT, acc_x_i, acc_y_i, acc_z_i, B, L, magn_x_i, magn_y_i, magn_z_i)
    ...
    theta.append(theta0 * 180 / math.pi), gam.append(gam0 * 180 / math.pi)
    Kgyr.append(K0 * 180 / math.pi), Kmagn.append(K0 * 180 / math.pi)
    V_E.append(V_Ei), V_N.append(V_Ni), V_h.append(V_hi)
    dS_E.append(dS_Ei), dS_N.append(dS_Ni), dS_h.append(dS_hi)
    dB.append(0), dL.append(0)

    # Формирование начальной матрицы C
    ...
    # Начальные значения сигналов коррекции
    if flag_alg == '1':
